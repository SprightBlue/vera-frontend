...
function Sidebar() {
    ...
    const menuItems: NavItem[] = [
        ...
        ...(user?.role === 'PROTECTED' ? [
            ...
            { label: "CentroIA", path: "/ai-center", icon: Sparkles, id: "nav-ai-center" },
            { label: "Alertas", path: "/alerts", icon: ShieldAlert, id: "nav-alertas" },
        ] : [])
    ];
    ...
    const renderNavLink = (item: NavItem) => {
        ...
        return (
            <Link
                ...
                to={item.path}
                id={item.id}
                className={`flex items-center gap-4 px-4 py-3 rounded-xl transition-all duration-200 text-[clamp(0.85rem,1vw,0.95rem)] font-medium border w-full justify-center xl:justify-start px-2 xl:px-4
                ${isActive
        ...
        <aside className="fixed left-0 top-0 h-screen bg-[#0c1020] border-r border-[#161f38] flex flex-col z-50 transition-all duration-300 w-20 xl:w-56">
            <div className="shrink-0 h-[5.6rem] flex items-center justify-center xl:justify-start px-6 border-b border-[#161f38]">
                <img
                    src={logoVeraIcon}
                    alt="Vera"
                    className="w-16 h-auto object-contain xl:hidden"
                />
                <img
                    src={logoVera}
                    alt="Vera"
                    className="hidden xl:block w-[70%] max-w-34 min-w-10 h-auto object-contain"
                />
            </div>

            <nav className="flex-1 flex flex-col gap-1.5 p-2 xl:p-4">
                {menuItems.map(renderNavLink)}
            </nav>

            <div className="shrink-0 flex flex-col gap-1.5 p-2 xl:p-4 border-t border-[#161f38]">
