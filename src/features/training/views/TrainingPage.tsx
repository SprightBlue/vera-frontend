                            ...
                            <div className="flex gap-2 flex-wrap">
                                {persons.map((p) => (
                                    <button
                                        ...
                                        className={`flex items-center gap-2.5 pl-2 pr-4 py-2 rounded-xl border text-sm font-medium transition-all cursor-pointer ${selectedPersonId === p.id ? "bg-violet-600/10 border-violet-500/30 text-white" : "bg-[#0d1222] border-[#182033] text-slate-400 hover:text-white hover:border-[#2a3550]"}`}
                                    >
                                        <img
                                            src={`https://ui-avatars.com/api/?name=${encodeURIComponent(p.fullName)}&background=7c3aed&color=fff&size=28`}
                                            alt={p.fullName}
                    ...
                    {loading ? (
                        ...
                        <div className="flex flex-col items-center justify-center pt-24 gap-4 text-center">
                            ...
                            <button
                                ...
                                className="flex items-center gap-2 px-5 py-2.5 bg-violet-600 hover:bg-violet-700 rounded-xl text-sm font-semibold text-white transition-all cursor-pointer"
                            >
                                <Send size={14} />
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="flex items-center justify-between">
                                ...
                                <button
                                    onClick={() => setShowAssignModal(true)}
                                    className="flex items-center gap-2 px-4 py-2 bg-violet-600 hover:bg-violet-700 rounded-xl text-xs font-semibold text-white transition-all cursor-pointer"
                                >
                                    <Send size={13} />
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
                                ...
                                {/* Banner */}
                                <div className="relative overflow-hidden bg-gradient-to-br from-violet-900/40 to-indigo-900/30 border border-violet-500/20 rounded-2xl p-6 flex flex-col justify-between min-h-[180px]">
                                    <div className="absolute right-0 top-0 h-full w-32 opacity-10">
                                        <BookOpen size={120} className="text-violet-300 absolute right-2 top-4" />
                                    </div>
                                    <div>
                                    ...
                                    <button
                                        onClick={() => setShowAssignModal(true)}
                                        className="mt-4 self-start flex items-center gap-2 px-4 py-2 bg-violet-600 hover:bg-violet-700 rounded-xl text-xs font-semibold text-white transition-all cursor-pointer"
                                    >
                                        Ver cómo funciona →
                                    </button>
                                </div>
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
                                ...
                                <div className="bg-[#0d1222] border border-[#182033] rounded-2xl p-5">
                                    <div className="flex items-center gap-2 mb-4">
                                        ...
                                        <div>
                                            <h3 className="text-sm font-semibold text-slate-200 inline">
                                                Misiones recomendadas{" "}
                                            </h3>
