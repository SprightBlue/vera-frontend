import {
    ...
    UI_VARIANTS_MAP,
    type UIVariant
} from '@/features/shared/utils/styleConfig';
                        ...
                        <button
                            ...
                            transition-all duration-200 cursor-pointer select-none active:scale-[0.97] shadow-md relative group overflow-hidden ${
                                isActive
                                    ...
                                    : `${UI_TOGGLE_INACTIVE} text-slate-400 hover:text-white`
                            }`}
                        >
                            <div
                                className={`absolute top-0 left-0 right-0 h-px bg-linear-to-r from-transparent to-transparent pointer-events-none transition-all duration-200 z-10 ${
                                    isActive ? `${styleConfig.laserColor}/30` : `via-transparent group-hover:${styleConfig.laserColor}/20`
                                }`}/>

                            <div
                                className={`absolute -top-5 -right-5 w-12 h-12 rounded-full filter blur-md pointer-events-none transform origin-top-right transition-all duration-300 ease-out z-0 ${
                                    isActive
                                        ? `opacity-20 scale-110 ${styleConfig.glowColor}`
                                        : `opacity-0 scale-75 ${styleConfig.glowColor} group-hover:opacity-10 group-hover:scale-110`
                                }`}/>

                            <span className="relative z-10 transition-colors duration-200">{option.label}</span>
                        </button>
                    );
                })}
            </div>
