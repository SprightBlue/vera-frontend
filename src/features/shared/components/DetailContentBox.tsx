...
export function DetailContentBox({ title, content, variant = 'info' }: DetailContentBoxProps) {
    ...
    return (
        <div className="space-y-2.5 flex flex-col flex-1">
            <h4 className={`text-[10px] font-sans font-bold uppercase tracking-wider flex items-center gap-2 ${config.textColor} select-none`}>
            ...
            rounded-r-lg rounded-l-none p-[clamp(1rem,1.3vw,1.6rem)] flex-1 ring-1 ring-inset ring-[#161f35]/20 shadow-md shadow-black/10`}>
                <p className="text-[clamp(0.78rem,0.84vw,0.92rem)] font-sans leading-relaxed font-medium select-text whitespace-pre-line text-slate-300 tracking-wide">
                    {content}
                </p>
            </div>
        </div>
