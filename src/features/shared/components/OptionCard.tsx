...
export function OptionCard({ title, description, to, buttonLabel, variant }: OptionCardProps) {
    ...
    return (
        ...
        >
            <div className={`absolute -top-20 -right-20 w-[clamp(220px,22vw,350px)] h-[clamp(220px,22vw,350px)] rounded-full ${config.glowColor} filter blur-3xl opacity-20 pointer-events-none transform origin-top-right transition-opacity duration-300`} />

            <div className={`absolute top-0 left-0 right-0 h-0.5 bg-linear-to-r from-transparent via-${variant === 'neutral' ? 'slate-500' : variant}-500/30 to-transparent pointer-events-none z-10`} />

            <div className="relative z-10 w-full space-y-4">

                <div className="flex items-center gap-2 select-none">
                    <span className={`w-2 h-2 rounded-full shadow-[0_0_8px_rgba(255,255,255,0.2)] ${variantDotColor}`} />
                    <span className="text-[clamp(10px,0.55vw,11.5px)] font-sans font-black tracking-widest text-slate-500 uppercase">
                        Módulo • Activo
                    </span>
                </div>
