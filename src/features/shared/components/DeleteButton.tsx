...
export function DeleteButton({
    ...
    return (
        ...
        >
            ...
            <div className={`absolute -top-5 -right-5 w-10 h-10 rounded-full filter blur-md pointer-events-none transform origin-top-right transition-all duration-500 ease-out z-0 bg-slate-400 ${
                isProcessing
                    ? "opacity-5 scale-125"
            ...
            {isProcessing ? (
                ...
                <Icon className="w-3.5 h-3.5 relative z-10 transition-transform duration-300 group-hover/btn:scale-105" strokeWidth={2.5} />
            )}
        </button>
    );
}